...
CGUICheckBox::CGUICheckBox(GUI_PARAMS* params, GUI_COLORS* colors, GUI_SPRITES* sprites, UINT _event, void* _dest, bool checked)
: CGUIBase(params, colors, sprites){
	m_bChecked=checked;
	m_Type=GUITYPE_CHECKBOX;
	m_Event=_event;
	m_pDest=_dest;
};
CGUICheckBox::~CGUICheckBox(){};

bool CGUICheckBox::WndProc( HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam ){
	if(!m_bEnabled)
		return false;
	switch(uMsg){
		// Keyboard messages
...
bool CGUICheckBox::HandleMouse( UINT uMsg, WPARAM wParam, LPARAM lParam ) {
...
bool CGUICheckBox::RequestFocus() {
	if(m_pParent){
		if(CanHaveFocus()){
...
bool CGUICheckBox::CanHaveFocus(){ return true; }
void CGUICheckBox::OnFocusIn() { m_bHasFocus = true; }
void CGUICheckBox::OnFocusOut() { m_bHasFocus = false; }
void CGUICheckBox::OnMouseEnter() { 
//	SetCursor(g_pCursor);
	m_bMouseOver = true; 
}
void CGUICheckBox::OnMouseLeave() { m_bMouseOver = false; }
void CGUICheckBox::OnHotkey() {
	m_bChecked=!m_bChecked;
	if(OnGUIEvent)
		OnGUIEvent(m_Event, this, m_pDest);
}

void CGUICheckBox::Step(){}
void CGUICheckBox::Draw(CGraphicsHandler *g){
	if(GetVisible()){
		UpdateBB();
		if(m_pSprites){
			...
			if(GetChecked())
				g->DrawTextTL(L"X", Vector2(m_pos.x+5, m_pos.y+2), m_pColors->Text);
		}
